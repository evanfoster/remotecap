...
async def run_client(user: str, host: str, command: str, cap_file: Path, keys: List[Path], password: str = None,
    ...
    async with semaphore:
        async with aiofiles.open(str(cap_file), mode='wb', buffering=0) as cap_fd, aiofiles.open(str(log_file),
                                                                                                 mode='wb',
                                                                                                 buffering=0) as log_fd:
            ...
            async with asyncssh.connect(host, port, client_keys=client_keys, username=user,
                ...
                await connection.run(command, input=sudo_password, stdout=cap_fd, stderr=log_fd, encoding=None,
                                     check=True)
